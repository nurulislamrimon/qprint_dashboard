...
const DeliveredHome = () => {
  const { data: receivedData, isLoading } = usePrintingRequestsQuery("orderStatus.status=Delivered");
          ...
          <tbody>
            {
              ...
              isLoading ? ([...Array(15)].map((_, index) => {
                ...
                receivedData?.data?.map((data: any, index: number) => (
                  <tr
                    onClick={() => setOpenModal(data?._id)}
                    key={index.toString()}
                    className="hover:bg-table-row-hover cursor-pointer transition-all text-center bg-white"
                  >
                    <PrintingTableRow data={data} />
