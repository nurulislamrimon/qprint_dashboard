...
const CustomerTableLayout = ({ data, isLoading }: any) => {
  // console.log(data);
  ...
  return (
    <div className="md:border rounded-md">
      <div className="mb-3 md: flex items-center gap-3 md:p-8">
        <span className="text-base md:text-lg font-medium text-black">Customer Table</span>
      ...
      <table className="w-full">
        <thead className="border-b text-center sticky z-50 top-0">
          <tr>
            {
              ["SL", "Order ID", "Total", "Status", "Action"].map((th, index) => {
                return (
                  <th
                    key={index}
                    className={`text-black-opacity-50 py-2.5 text-sm md:text-base font-normal text-center ${th === "Status" && "md:table-cell hidden"}`}>
                    {th}
                  </th>
                )
              })
        ...
        <tbody className="text-center">
          {
            isLoading ? (
              ...
              data?.data?.map((data: any, index: number) => (
                <tr key={index} className=" hover:bg-table-row-hover transition-all cursor-pointer" onClick={() => setOpenDrawerId(data?._id)} >
                  <SingleCustomerViewPageLayout data={data} key={index} index={index} />
                </tr>
              ))
          }
          {
